...
void loop() {
  ...
  if ( timeElapsed > minMillisElapsedBeforeFood ) {  
    ...
    // Relay default state is set to 'Normally Closed' (i.e. complete circuit - light is on / receiving power). 
    // Setting LOW is equivalent to turning relay to default state 
    ...
    Serial.println("--- numValuesToCollect");
    Serial.println(numValuesToCollect);
    ...
    if( averageSoundLevel > minSoundLevelBeforeFood ) {
        ...
        // Relay default state is set to 'Normally Closed' (i.e. complete circuit - light is on / receiving power). 
        // Setting HIGH is equivalent to turning relay to opposite  of default state. i.e. light is off. 
